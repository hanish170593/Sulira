...
class MemberProfileViewModel : ViewModel() {
    ...
    lateinit var memberProfileModel: MemberProfileModel
    lateinit var message: String

    @SuppressLint("CheckResult")
    fun getMemberUserInfo(userId: String) {
        userDataManager.getPremiumMemberUserInfo(userId)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeWith(object : DisposableObserver<MemberProfileModel>() {
                override fun onComplete() {

                }

                override fun onNext(response: MemberProfileModel) {
                    memberProfileModel = response
