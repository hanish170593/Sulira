...
class HealthConditionViewModel:ViewModel() {

    var tag = HealthConditionViewModel::class.java.simpleName!!
    private var userDataManager: UserDataManager = UserDataManager()
    val successful: MutableLiveData<Boolean> = MutableLiveData()
    lateinit var healthConditionModel: HealthConditionModel
    lateinit var message: String

    @SuppressLint("CheckResult")
    fun getHealthConditionList(){
        userDataManager.getHealthConditionModel()
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeWith(object: DisposableObserver<HealthConditionModel>(){
                override fun onComplete() {

                }

                override fun onNext(response: HealthConditionModel) {
                    healthConditionModel=response
