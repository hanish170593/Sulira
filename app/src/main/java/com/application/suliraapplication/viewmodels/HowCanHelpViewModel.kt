...
class HowCanHelpViewModel:ViewModel() {
    var tag = HowCanHelpViewModel::class.java.simpleName!!
    private var userDataManager: UserDataManager= UserDataManager()
    val successful:MutableLiveData<Boolean> = MutableLiveData()
    lateinit var howCanHelpModel: HowCanHelpModel
    lateinit var message: String

    @SuppressLint("CheckResult")
    fun getHowCanHelpList(){

        userDataManager.getHowCanHelpData()
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeWith(object:DisposableObserver<HowCanHelpModel>(){
                override fun onComplete() {

                }

                override fun onNext(response: HowCanHelpModel) {
                  howCanHelpModel=response
