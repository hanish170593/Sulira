...
import com.application.suliraapplication.models.GetHomeModel
...
class HomeDataViewModel:ViewModel() {
    ...
    var tag = HomeDataViewModel::class.java.simpleName!!
    private var userDataManager: UserDataManager = UserDataManager()
    val successful: MutableLiveData<Boolean> = MutableLiveData()
    lateinit var homeModel: GetHomeModel
    lateinit var message: String

    @SuppressLint("CheckResult")
    fun getHomeData(userId:String){
        userDataManager.getHomeData(userId)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeWith(object: DisposableObserver<GetHomeModel>(){
                override fun onComplete() {

                }

                override fun onNext(response: GetHomeModel) {
                    homeModel=response
