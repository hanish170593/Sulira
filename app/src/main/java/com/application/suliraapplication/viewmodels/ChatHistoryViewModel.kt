...
class ChatHistoryViewModel : ViewModel() {
    ...
    var tag = ChatHistoryViewModel::class.java.simpleName!!
    private var userDataManager: UserDataManager = UserDataManager()
    val successful: MutableLiveData<Boolean> = MutableLiveData()
    lateinit var chatHistoryModel: ChatHistoryModel
    lateinit var message: String

    @SuppressLint("CheckResult")
    fun getChatHistory(chatRef: String) {
        userDataManager.getChatHistory(chatRef)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeWith(object : DisposableObserver<ChatHistoryModel>() {
                override fun onComplete() {

                }

                override fun onNext(response: ChatHistoryModel) {
