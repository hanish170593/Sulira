...
class GetSavedCardListViewModel : ViewModel() {

    var tag = GetSavedCardListViewModel::class.java.simpleName!!
    private var userDataManager: UserDataManager = UserDataManager()
    val successful: MutableLiveData<Boolean> = MutableLiveData()
    lateinit var cardListModel: CardListModel
    lateinit var message: String

    @SuppressLint("CheckResult")
    fun getSavedCardsList(stripeToken: String) {
        userDataManager.getSavedCardsList(stripeToken)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeWith(object : DisposableObserver<CardListModel>() {
                override fun onComplete() {

                }

                override fun onNext(response: CardListModel) {
