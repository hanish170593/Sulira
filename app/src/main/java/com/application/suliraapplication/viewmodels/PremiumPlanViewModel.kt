...
class PremiumPlanViewModel : ViewModel() {

    var tag = PremiumPlanViewModel::class.java.simpleName!!
    private var userDataManager: UserDataManager = UserDataManager()
    val successful: MutableLiveData<Boolean> = MutableLiveData()
    lateinit var premiumPlanModel: PremiumPlanModel
    lateinit var message: String

    @SuppressLint("CheckResult")
    fun getPlan() {
        userDataManager.getPlan()
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeWith(object : DisposableObserver<PremiumPlanModel>() {
                override fun onComplete() {

                }

                override fun onNext(response: PremiumPlanModel) {
                    premiumPlanModel = response
