...
import com.application.suliraapplication.models.PolicyModel
...
class PolicyViewModel:ViewModel() {
    ...
    var tag = PolicyViewModel::class.java.simpleName!!
    private var userDataManager: UserDataManager = UserDataManager()
    val successful: MutableLiveData<Boolean> = MutableLiveData()
    lateinit var policyModel: PolicyModel
    lateinit var message: String

    @SuppressLint("CheckResult")
    fun policyData(){
        userDataManager.termsAndPrivacy()
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeWith(object: DisposableObserver<PolicyModel>(){
                override fun onComplete() {

                }

                override fun onNext(response: PolicyModel) {
