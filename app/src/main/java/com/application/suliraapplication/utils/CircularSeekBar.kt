...
class CircularSeekBar : View {
     ...
     * Holds the color value for `mPointerHaloPaint` before the `Paint` instance is created.
     */
    protected var mPointerHaloColorOnTouch =
     ...
     */
    protected var mCircleProgressColor =
     ...
     */
    protected var mPointerAlphaOnTouch =
    ...
    protected fun initAttributes(attrArray: TypedArray) {
        ...
        mMax =
            attrArray.getInt(R.styleable.CircularSeekBar_max, DEFAULT_MAX)
    ...
    protected fun setProgressBasedOnAngle(angle: Float) {
        ...
        mProgress =
            Math.round(mMax.toFloat() * mProgressDegrees / mTotalCircleDegrees)
    }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        touchAngle = (Math.atan2(
            y.toDouble(),
            x.toDouble()
        ) / Math.PI * 180 % 360).toFloat() // Verified
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                val pointerRadiusDegrees =
                    (mPointerRadius * 180 / (Math.PI * Math.max(
                        mCircleHeight,
                        mCircleWidth
                    ))).toFloat()
    ...
    companion object {
        ...
        protected val DEFAULT_CIRCLE_PROGRESS_COLOR =
            Color.argb(235, 74, 138, 255)
        protected val DEFAULT_POINTER_COLOR = Color.argb(235, 74, 138, 255)
        protected val DEFAULT_POINTER_HALO_COLOR =
            Color.argb(135, 74, 138, 255)
        protected val DEFAULT_POINTER_HALO_COLOR_ONTOUCH =
            Color.argb(135, 74, 138, 255)
