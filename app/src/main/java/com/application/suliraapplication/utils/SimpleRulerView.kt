...
class SimpleRulerView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : View(context, attrs), GestureDetector.OnGestureListener {
    /**
     ...
     */
    private var mRulerColor = Color.BLACK
     ...
     * texts for show
     */
    private var mTextList: List<String>? = null
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        if (typedArray != null) {
            ...
            mRulerColor = typedArray.getColor(
            ...
            mIntervalDis =
                typedArray.getDimension(R.styleable.simpleRulerView_intervalDistance, mIntervalDis)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in start until end) {
            ...
            if (i == mSelectedIndex) {
                mRulerPaint!!.color = mHighlightColor
            } else {
            ...
            if (mRulerCount > 0 && i >= 0 && i < mRulerCount) {
                ...
                if (i % 10 == 0) {
                    ...
                    text = if (mTextList != null && mTextList!!.size > 0) {
                        val index = i / 10
    ...
    private fun adjustPosition() {
        val scrollX = scrollX
        val dx = (mSelectedIndex * mIntervalDis - scrollX
                - mMaxOverScrollDistance)
     ...
     * @param offsetX
     */
    private fun refreshSelected(offsetX: Int) {
    ...
    fun setSelectedIndex(selectedIndex: Int) {
        ...
        post {
            scrollTo(
                (mSelectedIndex * mIntervalDis - mMaxOverScrollDistance).toInt(),
                0
            )
            invalidate()
            refreshSelected()
    ...
    fun setHighlightColor(mHighlightColor: Int) {
        this.mHighlightColor = mHighlightColor
    }

    fun getMarkTextColor(): Int {
        return mTextColor
    }

    fun setMarkTextColor(mMarkTextColor: Int) {
        mTextColor = mMarkTextColor
    }

    fun getMarkColor(): Int {
        return mRulerColor
    }

    fun setMarkColor(mMarkColor: Int) {
        mRulerColor = mMarkColor
    }
     ...
     * @param mTextList
     */
    fun setTextList(mTextList: List<String>?) {
     ...
     * @param mMinValue
     */
    fun setMinValue(mMinValue: Float) {
        this.mMinValue = mMinValue
        calculateTotal()
        invalidate()
    }

    fun getIntervalValue(): Float {
        return mIntervalValue
    }

    fun setIntervalValue(mIntervalValue: Float) {
        this.mIntervalValue = mIntervalValue
        calculateTotal()
        invalidate()
    }

    fun getIntervalDis(): Float {
        return mIntervalDis
    }

    fun setIntervalDis(mIntervalDis: Float) {
        this.mIntervalDis = mIntervalDis
    }

    fun getOnValueChangeListener(): OnValueChangeListener? {
        return onValueChangeListener
    }

     fun setOnValueChangeListener(
